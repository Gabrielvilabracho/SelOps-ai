...
- **Scoop** is the supported Windows install path for SelOps.
...
- **GGA on Windows** works from both Git Bash and PowerShell. selops installs a `gga.ps1` shim that automatically delegates to Git Bash, so no manual shell switching is required.
...
| Pi | `%USERPROFILE%\.pi\` (Pi config, project agents/chains, SelOps support assets) |
