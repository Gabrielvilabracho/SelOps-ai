...
- [ ] T-07 Create `internal/agentbuilder/registry.go` — implement `Registry` struct with `Load(path)`, `Save(path)`, `Add(entry)`, `List()` methods; JSON at `~/.config/selops-ai/custom-agents.json`; `version: 1` field; conflict detection for built-in and custom name collisions
