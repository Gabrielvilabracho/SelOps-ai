...
- **Decommission is a planned event, not a shutdown.** A model is not simply switched off when it is no longer needed. Decommission is a defined procedure: stakeholders are notified, a deprecation window is enforced, data flows are redirected, and records are preserved before the model is removed. (NIST GOVERN 1.7 [UNVERIFIED: exact subcategory] — model decommissioning and retirement.)
- **Record preservation is a regulatory obligation.** The records that accompany a model — training data summaries, evaluation results, approval decisions, incident logs — must be preserved after decommission for a defined retention period. These records are required for post-incident investigations, audits, and regulatory reviews. Destroying them prematurely is a compliance failure. (ISO/IEC 42001:2023 A.6.2.8 [UNVERIFIED: exact clause] — AI system records and retention.)

## Patterns

### Model Version Control Record
For each model version in production, maintain a version record containing: (1) model identifier — name, version string, provider, (2) deployment date and approving party, (3) what this version replaced and why, (4) behavioral baseline — linked benchmark results or evaluation report, (5) known limitations or deviations from the previous version, (6) expected replacement trigger — the condition or date that will initiate the next lifecycle event, (7) current lifecycle status — active / deprecated / decommissioned. The record lives in source control or a governed artifact store; it is not a wiki entry that can be silently edited. Grounded in NIST GOVERN 1.6 [UNVERIFIED: exact subcategory] model inventory requirements and ISO/IEC 42001:2023 A.6.2.3 [UNVERIFIED: exact clause] (AI system documentation).
...
When a model version is to be decommissioned (NIST GOVERN 1.7 [UNVERIFIED: exact subcategory]):
...
- NIST AI RMF GOVERN function — AI governance lifecycle controls; GOVERN 1.6 (model inventory and versioning) [UNVERIFIED: exact subcategory], GOVERN 1.7 (decommissioning and retirement) [UNVERIFIED: exact subcategory]
- ISO/IEC 42001:2023 — AI management system standard; A.6.2.3 (AI system documentation) [UNVERIFIED: exact clause], A.6.2.8 (AI system records and retention) [UNVERIFIED: exact clause]
