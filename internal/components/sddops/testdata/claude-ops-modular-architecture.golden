...
### Model SBOM and Provenance Record
...
- [ ] Is a new model being introduced? If yes, create or update the model SBOM entry before merging.
- [ ] Is an existing model being updated? If yes, validate the new checksum, compare behavioral baseline, and get approval before promoting.
...
- [ ] Model SBOM complete for every model in production
- [ ] Supply chain controls (version pinning, checksum verification, update gating) verified and documented
...
Supply chain risk is the third and often underestimated failure mode. An AI system's external models are a dependency just as package registries are — but with less community scrutiny and fewer verification tools. A compromised model adapter, a silently-updated API endpoint, or a downloaded fine-tune from an unverified source can alter system behavior without touching a single line of code. The SBOM and supply chain controls patterns exist to make this risk visible and manageable before it becomes a production incident.
