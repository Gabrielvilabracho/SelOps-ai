...
func TestRunArgs_UpgradeToolFilter(t *testing.T) {
	...
	if strings.Contains(out, "selops") && !strings.Contains(out, "engram") {
...
func TestRenderUpgradeReport_PerToolSemantics_Deterministic(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantContains:   []string{"selops", "[--]", "source build"},
			...
			results: []upgrade.ToolUpgradeResult{
				{
					...
					NewVersion: "1.5.0",
					Status:     upgrade.UpgradeSkipped,
					ManualHint: "Download from https://github.com/Gabrielvilabracho/selops-ai/releases",
				},
			},
			wantContains:   []string{"selops", "manual update required", "github.com", "[--]"},
			wantNotContain: []string{"[!!]", "FAILED"},
		},
		{
		...
		},
		{
			...
			wantContains:   []string{"engram", "[ok]", "selops", "[--]", "gga", "1 succeeded", "2 skipped"},
			wantNotContain: []string{"FAILED", "[!!]"},
		},
	}
